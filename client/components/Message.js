...
import { fetchFriends, saveMessage, sendMessage } from '../store'
...
class Message extends Component {
  ...
  handleSubmit (evt) {
    ...
    else this.props.singleMessage && this.props.singleMessage.messageId ? this.props.handleSend({ content: evt.target.content.value, messageId: this.props.singleMessage.id, sender: this.props.user }) : this.props.handleSend({ sender: this.props.user, receiver: this.props.friends[evt.target.friend.value], content: evt.target.content.value })
  }
          ...
          <div className="form-group row">
            ...
            <select name="friend">
              { this.props.friends && this.props.friends.map((friend, index) => (
                <option value={index} key={friend.id}>{friend.username}</option>
              ))}
            </select>
            </label>
          </div>
